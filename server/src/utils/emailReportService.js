...
const generateDailyReportData = async () => {
  ...
  // 3. Compute payment method sales breakdown
  let cashSales = 0;
  let cardSales = 0;
  let creditSales = 0;
  ...
  const bakeryProducts = await Product.find({
    category: { $regex: /bakery/i },
    isActive: true
  });
  const bakeryWasteItemsDeducted = bakeryProducts.reduce((acc, p) => acc + (p.quantityInStock || 0), 0);
  const bakeryWasteValue = bakeryProducts.reduce((acc, p) => acc + ((p.quantityInStock || 0) * (p.sellingPrice || 0)), 0);
...
const sendDailyReportEmail = async (reportData) => {
  ...
  // Use defensive timeouts to prevent blocking SMTP requests
    ...
    <html>
      ...
      <body>
        <div class="container">
          ...
          <div class="grid">
            <div class="card">
              <div class="card-title">Total Revenue Today (මුළු ආදායම)</div>
          ...
          <ul class="list">
            <li class="list-item">
              <span class="item-name">CASH Sales (මුදල්)</span>
              <span class="item-value" style="color: #e2e8f0;">Rs. ${reportData.cashSales.toFixed(2)}</span>
            </li>
            <li class="list-item">
              <span class="item-name">CARD Sales (කාඩ්පත්)</span>
              <span class="item-value" style="color: #e2e8f0;">Rs. ${reportData.cardSales.toFixed(2)}</span>
            </li>
            <li class="list-item">
              <span class="item-name">CREDIT (ණය Book) Sales</span>
              <span class="item-value" style="color: #e2e8f0;">Rs. ${reportData.creditSales.toFixed(2)}</span>
            </li>
          </ul>

          <div class="section-title">Inventory Adjustments</div>
          <ul class="list">
            <li class="list-item">
              <span class="item-name">Bakery Waste Items Deducted</span>
              <span class="item-value" style="color: #fbbf24;">${reportData.bakeryWasteItemsDeducted} units (Valued: Rs. ${reportData.bakeryWasteValue.toFixed(2)})</span>
            </li>
          </ul>
...
const startReportScheduler = () => {
  // EOD cron job scheduled at 11:59 PM nightly (59 23 * * *)
